...
在很长一段时间里，浏览器的接口没有一个统一的组织进行规范，导致各个浏览器的`API`不相同，给开发带来了很大的负担，比如现在的`W3C`,直到现在还有一些常见的兼容代码：
...
从`JQuery`和原生代码的实现来看，**开发者完全不用在写代码时，考虑特定浏览器某个API的兼容性，这是巨大的进步**
...
**随着`ajax`技术的发展和网页交互越来越复杂,以表单为例常见的JQuery代码如下：**
...
隔离变化是软件工程中的精髓思想，下列是基于`JQuery`从单一交互变为复杂交互代码依赖图：
...
1. 随着交互复杂度上升，如何管理代码与视图复杂的交互依赖关系

2. 如何建立代码与视图的显式关系

3. 如何最大程度降低视图代码的体积

`AngularJS`基于数据驱动视图的思想，**利用模板将代码和视图建立起显式的关联，而且把更新DOM的流程隐藏到框架之内，同时有效降低了这视图与交互不必要的代码增长**，2、3点很容易就能看到`AngularJS`解决了，这里我们重点关注第一点,**随着交互复杂度上升，如何管理代码与视图复杂的交互依赖关系**
...
因为`AngularJS`没有提供组件这个概念，导致`UI`代码复用率很低
...
https://www.infoq.cn/article/2014/05/facebook-mvc-flux?utm_source=related_read_bottom&utm_medium=article

http://qingbob.com/posts/
http://qingbob.com/arch-enjoy_angular-architecture-best-practices-zh-cn/

http://qingbob.com/fe_arch_005_mvp/http://qingbob.com/fe_arch_005_mvp/

http://qingbob.com/mvc-vs-flux/

http://qingbob.com/starting-from-limitations-of-mvc-in-front-end-development/
