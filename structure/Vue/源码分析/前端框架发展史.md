...
这带来调试和理解的困难，比如调试代码。假设在一个复杂的`MVC`的架构中，有多个`controller`可以修改`model`，而开发时`model`的数据产出并非如你所愿，则你很难判断出是哪个`controller`出的错，只能使用控制变量法进行调试
...
毫无疑问，随着项目复杂度上升，那么整个数据流图会变成一张网，难以理解

如果我们引入一个全局对象呢

![](./unidirectionaldataflow.png)

把共享状态提升到全局对象之后，我们的数据流变得清晰
