...
关于eslint由来我推荐这篇文章，[ESLint发展史](https://zhuanlan.zhihu.com/p/32297243)

简单介绍一下eslint，一个高度定制化的lint工具，这里我们直接上配置文件,这里配置了js和vue的，如果是TS的话，可以自行解决
...
有了eslint为什么需要prettier呢，eslint主要还是作为代码质量检测的工具，他对风格的管理还是偏弱，而prettier专用于风格管理所以我们需要将两者结合起来，这里直接上配置
...
这里我们让prettier接管风格部分的管理

``` shell
  npm i -D eslint prettier eslint-config-prettier  eslint-plugin-prettier eslint-plugin-vue @vue/eslint-config-prettier  
```
...
```js
function getGitAddedFile(path = './', lintReg = /(\.js|\.jsx)(\n|$)/gi) {
    const { exec } = require('child_process');
    const GITDIFF = `git diff --cached --diff-filter=ACMR --name-only -- ${path}`;
    return new Promise((resolve, reject) => {
        exec(GITDIFF, (error, stdout) => {
            ...
            const diffFileArray = stdout
                .split('\n')
                .filter((diffFile) => lintReg.test(diffFile));
